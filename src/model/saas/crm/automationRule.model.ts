...
const actionSchema = new mongoose.Schema<IAutomationAction>(
  {
    type: {
      type: String,
      enum: [
        "send_whatsapp", "send_email", "move_stage",
        "assign_to", "add_tag", "remove_tag", "webhook_notify", "create_meeting",
      ],
      required: true,
    },
...
const automationRuleSchema: Schema<IAutomationRule> = new mongoose.Schema(
  {
    ...
    trigger: {
      type: String,
      enum: [
        "stage_enter", "stage_exit", "lead_created", "deal_won", "deal_lost",
        "score_above", "score_below", "no_contact", "tag_added", "tag_removed",
      ],
      required: true,
    },
    triggerConfig: {
...
export default AutomationRule;export { automationRuleSchema as AutomationRuleSchema };
