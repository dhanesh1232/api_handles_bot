...
export const PortfolioService = {
  ...
  getAgencyStats: async (agencyCode: string) => {
    ...
    const results = await Promise.all(
      clients.map(async (client) => {
        try {
          ...
          const s = stats[0] || { totalLeads: 0, totalValue: 0, wonCount: 0, wonValue: 0 };
          return {
            ...
            metrics: {
              ...
              wonCount: s.wonCount,
              wonValue: s.wonValue,
              conversionRate: s.totalLeads > 0 ? (s.wonCount / s.totalLeads) * 100 : 0
            }
          };
        } catch (err) {
          console.error(`Error aggregating stats for ${client.clientCode}:`, err);
          return null;
        }
      })
    );
    ...
    const totals = validResults.reduce((acc: any, curr: any) => {
