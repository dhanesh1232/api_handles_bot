...
export const runAutomations = async (
  ...
  const ruleQuery: mongoose.FilterQuery<IAutomationRule> = {
    clientCode, trigger: ctx.trigger, isActive: true,
  };
  if (ctx.stageId)  ruleQuery["triggerConfig.stageId"] = new mongoose.Types.ObjectId(ctx.stageId);
  ...
  await Promise.allSettled(rules.map((rule) => executeRule(clientCode, rule, ctx)));
};
...
const executeRule = async (
  ...
  for (const action of rule.actions) {
    if (action.delayMinutes === 0) {
      ...
      await enqueueDelayedAction(clientCode, action, ctx.lead, (rule as any)._id.toString());
    }
  }
...
const executeAction = async (
  ...
  switch (action.type) {
    case "send_whatsapp": {
      const { createWhatsappService } = await import("../whatsapp/whatsappService.ts");
      const service = createWhatsappService(null as any);
      const cfg = action.config as { templateName: string; variables?: string[] };
      await service.sendOutboundMessage(
        clientCode, "", undefined, undefined, undefined, "automation",
        cfg.templateName, "en_US", cfg.variables ?? [],
      );
      break;
    }
    case "send_email": {
      ...
      await service.sendEmail(clientCode, { to: lead.email ?? "", subject: cfg.subject, html: cfg.htmlBody });
      break;
    }
    case "move_stage": {
      const { moveLead } = await import("./lead.service.ts");
      const cfg = action.config as { stageId: string };
      await moveLead(clientCode, lead._id.toString(), cfg.stageId, "automation");
      break;
    }
    case "assign_to": {
      const { Lead } = await getCrmModels(clientCode);
      const assignTo = (action.config as { assignTo: string }).assignTo;
      await Lead.updateOne({ _id: lead._id, clientCode }, { $set: { assignedTo: assignTo } });
      await logActivity(clientCode, {
        leadId: lead._id.toString(), type: "lead_assigned",
        title: `Assigned to ${assignTo}`, performedBy: "automation",
      });
      break;
    }
    case "add_tag": {
      const { updateTags } = await import("./lead.service.ts");
      await updateTags(clientCode, lead._id.toString(), [(action.config as { tag: string }).tag], []);
      break;
    }
    case "remove_tag": {
      const { updateTags } = await import("./lead.service.ts");
    ...
    case "create_meeting": {
      const { createGoogleMeetService } = await import("../googleMeetService.ts");
...
const enqueueDelayedAction = async (
  ...
  await crmQueue.add(
    { clientCode, type: "crm.automation_action", payload: { actionType: action.type, actionConfig: action.config, leadId: lead._id.toString(), ruleId } },
...
function evaluateCondition(condition: IAutomationCondition, lead: ILead): boolean {
  if (!condition) return true;
  const value = condition.field.split(".").reduce(
    (obj: any, key: string) => (obj && typeof obj === "object" ? obj[key] : undefined),
    lead as any,
  );
  switch (condition.operator) {
    ...
    case "contains": return Array.isArray(value) && (value as string[]).includes(condition.value as string);
    default:         return true;
  }
}
...
export const getRules = async (clientCode: string): Promise<IAutomationRule[]> => {
  const { AutomationRule } = await getCrmModels(clientCode);
...
export const createRule = async (
  clientCode: string,
  input: Omit<IAutomationRule, "_id" | "clientCode" | "executionCount" | "lastExecutedAt" | "createdAt" | "updatedAt">,
...
export const updateRule = async (
  clientCode: string, ruleId: string, updates: Partial<IAutomationRule>,
): Promise<IAutomationRule | null> => {
  const { AutomationRule } = await getCrmModels(clientCode);
  return AutomationRule.findOneAndUpdate({ _id: ruleId, clientCode }, { $set: updates }, { new: true });
};

export const deleteRule = async (clientCode: string, ruleId: string): Promise<void> => {
  const { AutomationRule } = await getCrmModels(clientCode);
  await AutomationRule.deleteOne({ _id: ruleId, clientCode });
};

export const toggleRule = async (clientCode: string, ruleId: string): Promise<IAutomationRule | null> => {
  const { AutomationRule } = await getCrmModels(clientCode);
  const rule = await AutomationRule.findOne({ _id: ruleId, clientCode });
  if (!rule) return null;
  return AutomationRule.findByIdAndUpdate(ruleId, { $set: { isActive: !rule.isActive } }, { new: true });
};

export const testRule = async (
  clientCode: string, ruleId: string, leadId: string,
): Promise<{ passed: boolean; conditionResult: boolean; actionsWouldRun: string[] }> => {
  ...
  const conditionResult = rule.condition ? evaluateCondition(rule.condition, lead as any) : true;
  return { passed: conditionResult, conditionResult, actionsWouldRun: conditionResult ? rule.actions.map((a) => a.type) : [] };
};