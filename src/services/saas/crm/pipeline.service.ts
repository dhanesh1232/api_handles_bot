...
const DEFAULT_STAGE_TEMPLATES: Record<
  string,
  Array<{ name: string; color: string; probability: number; isWon?: boolean; isLost?: boolean }>
> = {
  sales: [
  ...
  appointment: [
    ...
    { name: "Appointment No Show",         color: "#10b981", probability: 100, isWon: true },
    { name: "Appointment Follow Up",        color: "#ef4444", probability: 0,   isLost: true },
  ],
  product_purchase:[
    { name: "New Lead",    color: "#6366f1", probability: 10 },
    { name: "Contacted",   color: "#3b82f6", probability: 25 },
...
export const getPipelines = async (clientCode: string): Promise<any[]> => {
  ...
  const pipelines = await Pipeline.find({ clientCode, isActive: true }).sort({ order: 1 }).lean();
  const stages = await PipelineStage.find({ clientCode }).sort({ order: 1 }).lean();

  return pipelines.map((p: any) => ({
    ...p,
    stages: stages.filter((s: any) => s.pipelineId.toString() === p._id.toString()),
  }));
};
...
export const getPipelineWithStages = async (
  clientCode: string, pipelineId: string,
): Promise<{ pipeline: IPipeline; stages: IPipelineStage[] } | null> => {
  const { Pipeline, PipelineStage } = await getCrmModels(clientCode);
  const pipeline = await Pipeline.findOne({ _id: pipelineId, clientCode, isActive: true });
  if (!pipeline) return null;
  const stages = await PipelineStage.find({ clientCode, pipelineId: pipeline._id }).sort({ order: 1 });
...
export const createPipeline = async (
  ...
  const rawStages =
    input.stages.length > 0
      ? input.stages
      : (DEFAULT_STAGE_TEMPLATES[templateKey ?? "sales"] ?? DEFAULT_STAGE_TEMPLATES.sales);
...
export const updatePipeline = async (
  ...
  return Pipeline.findOneAndUpdate({ _id: pipelineId, clientCode }, { $set: updates }, { new: true });
};

// ─── 5. Set default pipeline ─────────────────────────────────────────────────

export const setDefaultPipeline = async (clientCode: string, pipelineId: string): Promise<void> => {
  const { Pipeline } = await getCrmModels(clientCode);
  await Pipeline.updateMany({ clientCode }, { $set: { isDefault: false } });
  await Pipeline.findOneAndUpdate({ _id: pipelineId, clientCode }, { $set: { isDefault: true } });
};

// ─── 6. Soft delete pipeline ─────────────────────────────────────────────────

export const archivePipeline = async (clientCode: string, pipelineId: string): Promise<void> => {
  const { Pipeline } = await getCrmModels(clientCode);
  const pipeline = await Pipeline.findOne({ _id: pipelineId, clientCode });
  if (!pipeline) throw new Error("Pipeline not found");
  if (pipeline.isDefault) throw new Error("Cannot archive the default pipeline");
...
export const addStage = async (
  clientCode: string,
  pipelineId: string,
  input: { name: string; color?: string; probability?: number; isWon?: boolean; isLost?: boolean; insertAfterOrder?: number },
): Promise<IPipelineStage> => {
  const { Pipeline, PipelineStage } = await getCrmModels(clientCode);
  const pipeline = await Pipeline.findOne({ _id: pipelineId, clientCode });
  if (!pipeline) throw new Error("Pipeline not found");

  const lastStage = await PipelineStage.findOne({ clientCode, pipelineId }).sort({ order: -1 }).select("order");
  const newOrder = input.insertAfterOrder !== undefined ? input.insertAfterOrder + 1 : (lastStage?.order ?? -1) + 1;

  if (input.insertAfterOrder !== undefined) {
    await PipelineStage.updateMany({ clientCode, pipelineId, order: { $gte: newOrder } }, { $inc: { order: 1 } });
  }

  return PipelineStage.create({
    clientCode, pipelineId, name: input.name,
    color: input.color ?? "#3b82f6", order: newOrder,
    probability: input.probability ?? 0,
    isDefault: false, isWon: input.isWon ?? false, isLost: input.isLost ?? false, autoActions: [],
  });
};
...
export const updateStage = async (
  clientCode: string, stageId: string,
  updates: Partial<Pick<IPipelineStage, "name" | "color" | "probability" | "isWon" | "isLost" | "autoActions">>,
): Promise<IPipelineStage | null> => {
  const { PipelineStage } = await getCrmModels(clientCode);
  return PipelineStage.findOneAndUpdate({ _id: stageId, clientCode }, { $set: updates }, { new: true });
};

// ─── 9. Reorder all stages ───────────────────────────────────────────────────

export const reorderStages = async (
  clientCode: string, pipelineId: string, newOrder: UpdateStageOrderInput[],
): Promise<void> => {
  const { Pipeline, PipelineStage } = await getCrmModels(clientCode);
  const pipeline = await Pipeline.findOne({ _id: pipelineId, clientCode });
  if (!pipeline) throw new Error("Pipeline not found");
  await Promise.all(
    newOrder.map(({ stageId, newOrder: order }) =>
      PipelineStage.updateOne({ _id: stageId, clientCode, pipelineId }, { $set: { order } }),
    ),
  );
};
...
export const deleteStage = async (
  clientCode: string, stageId: string, moveleadsToStageId?: string,
): Promise<void> => {
  ...
  if (leadCount > 0 && !moveleadsToStageId) {
    throw new Error(`${leadCount} leads are in this stage. Provide moveleadsToStageId to migrate them first.`);
  }
  if (leadCount > 0 && moveleadsToStageId) {
    const targetStage = await PipelineStage.findOne({ _id: moveleadsToStageId, clientCode });
    if (!targetStage) throw new Error("Target stage not found");
    await Lead.updateMany({ clientCode, stageId }, { $set: { stageId: moveleadsToStageId } });
  }
...
export const getBoardSummary = async (clientCode: string, pipelineId: string): Promise<BoardColumn[]> => {
  const { Lead, PipelineStage } = await getCrmModels(clientCode);
  const stages = await PipelineStage.find({ clientCode, pipelineId }).sort({ order: 1 });
  if (stages.length === 0) return [];

  const agg = await Lead.aggregate([
    { $match: { clientCode, pipelineId: new mongoose.Types.ObjectId(pipelineId), isArchived: { $ne: true } } },
    { $group: { _id: "$stageId", leadCount: { $sum: 1 }, totalValue: { $sum: { $ifNull: ["$dealValue", 0] } } } },
  ]);

  const statsMap = new Map(agg.map((row: any) => [row._id.toString(), row]));
  return stages.map((stage) => {
    const stats: any = statsMap.get(stage._id.toString());
    return { stage, leadCount: stats?.leadCount ?? 0, totalValue: stats?.totalValue ?? 0 };
  });
};

// ─── 12. Revenue forecast ────────────────────────────────────────────────────

export const getRevenueForecast = async (clientCode: string, pipelineId: string): Promise<ForecastRow[]> => {
  const { Lead, PipelineStage } = await getCrmModels(clientCode);
  const stages = await PipelineStage.find({ clientCode, pipelineId, isLost: false }).sort({ order: 1 });
  if (stages.length === 0) return [];

  const agg = await Lead.aggregate([
    { $match: { clientCode, pipelineId: new mongoose.Types.ObjectId(pipelineId), status: "open", isArchived: { $ne: true } } },
    { $group: { _id: "$stageId", totalValue: { $sum: { $ifNull: ["$dealValue", 0] } }, leadCount: { $sum: 1 } } },
  ]);
  ...
  return stages.map((stage) => {
    ...
    return {
      stageId:  stage._id.toString(), stageName: stage.name, probability: stage.probability,
      totalValue, expectedRevenue: Math.round((totalValue * stage.probability) / 100),
...
export const getDefaultStage = async (clientCode: string, pipelineId: string): Promise<IPipelineStage | null> => {
  const { PipelineStage } = await getCrmModels(clientCode);
  return (
    (await PipelineStage.findOne({ clientCode, pipelineId, isDefault: true })) ??
...
export const getDefaultPipeline = async (clientCode: string): Promise<IPipeline | null> => {
  const { Pipeline } = await getCrmModels(clientCode);
  return (
...
export const duplicatePipeline = async (
  clientCode: string, sourcePipelineId: string, newName: string,
): Promise<{ pipeline: IPipeline; stages: IPipelineStage[] }> => {
  ...
  return createPipeline(clientCode, {
    name: newName, description: source.pipeline.description, isDefault: false,
    stages: source.stages.map((s) => ({ name: s.name, color: s.color, probability: s.probability, isWon: s.isWon, isLost: s.isLost })),
  });
};