...
// List all blueprints
router.get("/blueprints", verifyCoreToken, async (req: Request, res: Response) => {
  try {
...
router.post("/blueprints", verifyCoreToken, async (req: Request, res: Response) => {
  try {
    await dbConnect("services");
...
router.post("/blueprints/deploy", verifyCoreToken, async (req: Request, res: Response) => {
  try {
    const { clientCode, blueprintId } = req.body;
    const result = await OrchestratorService.deployBlueprint(clientCode, blueprintId, "agency_admin");
    res.json({ ...result });
  } catch (err: any) {
    res.status(500).json({ success: false, message: err.message });
  }
...
router.get("/portfolio/:agencyCode/stats", verifyCoreToken, async (req: Request, res: Response) => {
  try {
    const stats = await PortfolioService.getAgencyStats(req.params.agencyCode as string);
...
router.get("/portfolio/:agencyCode/health", verifyCoreToken, async (req: Request, res: Response) => {
  try {
    const report = await HealthService.checkPortfolioHealth(req.params.agencyCode as string);
...
router.get("/usage/:clientCode", verifyCoreToken, async (req: Request, res: Response) => {
  try {
    const usage = await UsageService.getUsage(req.params.clientCode as string);
...
router.get("/staff/:agencyCode", verifyCoreToken, async (req: Request, res: Response) => {
  try {
    await dbConnect("services");
