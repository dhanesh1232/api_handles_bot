...
router.post("/automations", async (req: Request, res: Response) => {
  try {
    ...
    const rule = await automationService.createRule(req.clientCode!, {
      name, trigger, triggerConfig: triggerConfig ?? {}, condition, actions, isActive: true,
...
router.patch("/automations/:ruleId", async (req: Request, res: Response) => {
  try {
    const rule = await automationService.updateRule(
      req.clientCode!, req.params.ruleId as string, req.body,
    );
    if (!rule) {
...
router.patch("/automations/:ruleId/toggle", async (req: Request, res: Response) => {
  try {
    const rule = await automationService.toggleRule(req.clientCode!, req.params.ruleId as string);
    if (!rule) {
      res.status(404).json({ success: false, message: "Rule not found" });
      return;
    }
...
router.delete("/automations/:ruleId", async (req: Request, res: Response) => {
  try {
    await automationService.deleteRule(req.clientCode!, req.params.ruleId as string);
 ...
 * Body: { leadId }
 */
router.post("/automations/:ruleId/test", async (req: Request, res: Response) => {
  try {
    ...
    const result = await automationService.testRule(
      req.clientCode!, req.params.ruleId as string, leadId,
    );
