 ...
 * 1. CRM - Get Leads
 */
router.get("/crm/leads", validateClientKey, async (req: AuthRequest, res: Response) => {
  try {
 ...
 */
router.patch("/crm/leads/:id/status", validateClientKey, async (req: AuthRequest, res: Response) => {
  try {
    const { clientCode } = req;
    if (!clientCode) return res.status(401).json({ error: "Unauthorized" });
 ...
 */
router.post("/meetings/create", validateClientKey, async (req: AuthRequest, res: Response) => {
  try {
    const { clientCode } = req;
    if (!clientCode) return res.status(401).json({ error: "Unauthorized" });
 ...
 */
router.post("/emails/campaign", validateClientKey, async (req: AuthRequest, res: Response) => {
  try {
    const { clientCode } = req;
    if (!clientCode) return res.status(401).json({ error: "Unauthorized" });
 ...
 */
router.post("/create/google-meet", validateClientKey, async (req: AuthRequest, res: Response) => {
  try {
    const { clientCode } = req;
    if (!clientCode) return res.status(401).json({ error: "Unauthorized" });
